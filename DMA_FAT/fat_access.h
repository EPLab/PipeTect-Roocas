...
struct fatfs
{
	...
	UINT32					empty_chunk_start_cluster_number;
	int						empty_chunk_used_counter;
};
...
int		fatfs_sector_writer(struct fatfs *fs, UINT32 Startcluster, UINT32 offset, unsigned char *target, UINT32* LatestCluster, unsigned char isChunkRequired);
void	fatfs_show_details(struct fatfs *fs);
UINT32	fatfs_get_root_cluster(struct fatfs *fs);
int		fatfs_get_file_entry(struct fatfs *fs, UINT32 Cluster, char *nametofind, FAT32_ShortEntry *sfEntry);
